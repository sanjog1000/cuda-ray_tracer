...
// -----------------------------------------------------------------------------
// Thin triangular prism: a real 3D solid built from 8 triangle faces.
// Each prism owns its internal triangle faces; the material is owned by
// create_extra_geometry()/clear_extra_geometry() exactly once.
// -----------------------------------------------------------------------------
class triangular_prism : public hittable {
public:
    triangle* faces[8];
    vec verts[6];
    ...
    triangular_prism(
        ...
        verts[3] = D; verts[4] = E; verts[5] = F;

        for(int i = 0; i < 8; ++i) faces[i] = nullptr;

        // Front / back triangular faces.
        faces[0] = new triangle(A, B, C, mat);
        faces[1] = new triangle(D, F, E, mat);

        // Three rectangular side walls, each split into two triangles.
        faces[2] = new triangle(A, D, E, mat);
        faces[3] = new triangle(A, E, B, mat);

        faces[4] = new triangle(B, E, F, mat);
        faces[5] = new triangle(B, F, C, mat);

        faces[6] = new triangle(C, F, D, mat);
        faces[7] = new triangle(C, D, A, mat);
    }

    __host__ __device__
    virtual ~triangular_prism(){
        for(int i = 0; i < 8; ++i){
            delete faces[i];
            faces[i] = nullptr;
        }
    ...
    virtual bool hit(
        ...
        for(int i = 0; i < 8; ++i){
            if(faces[i]->hit(r, t_min, closest_so_far, temp_rec, local_state)){
                hit_anything = true;
    ...
    virtual bool bounding_box(aabb& output_box) const override {
        ...
        for(int i = 1; i < 6; ++i){
            mn = vec(
                fminf(mn.x(), verts[i].x()),
                fminf(mn.y(), verts[i].y()),
                fminf(mn.z(), verts[i].z())
            );

            mx = vec(
                fmaxf(mx.x(), verts[i].x()),
                fmaxf(mx.y(), verts[i].y()),
                fmaxf(mx.z(), verts[i].z())
            );
        }

        // Tiny padding avoids zero-thickness AABB edge cases.
        const float eps = 1e-4f;
        output_box = aabb(
            vec(mn.x() - eps, mn.y() - eps, mn.z() - eps),
            vec(mx.x() + eps, mx.y() + eps, mx.z() + eps)
        );
        return true;
    }
};
...
__global__ void create_extra_geometry(
    ...
    // -----------------------------------------------------------------
    // THREE REAL 3D METALLIC TRIANGULAR PRISMS
    // -----------------------------------------------------------------
...
__global__ void create_volumetrics(
    ...
    fog_media[idx] = new constant_medium(fog_boundaries[idx], 0.001f, fog_materials[idx]);
    ++idx;
...
int main(){
    ...
    std::cout << "Architectural solids: " << NUM_EXTRA_SOLIDS
              ...
              << NUM_EXTRA_TRIANGLE_FACES << " triangle faces, "
              << NUM_EXTRA_TRIANGULAR_PRISMS << " triangular prisms)" << std::endl;
