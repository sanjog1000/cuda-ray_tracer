...
__global__ void create_extra_geometry(
    ...
    // 3. Back wall.
    extra_materials[m] = new lambertian(vec(0.33f, 0.35f, 0.39f));
    cuboid_boundaries[c] = new cuboid(
    ...
    for(int i = 0; i < 4; ++i){
        extra_materials[m] = new metal(vec(0.16f, 0.18f, 0.22f), 0.24f);
    ...
    for(int i = 0; i < 5; ++i){
        extra_materials[m] = new lambertian(vec(0.40f, 0.42f, 0.46f));
        cuboid_boundaries[c] = new cuboid(
    ...
    // Six metallic triangular panels form a deliberately separated sculpture:
    // three large front triangles plus three smaller dark inset triangles.
    // All six bases sit exactly on the right platform top (y = 2.35).
    // The front row is spread laterally so their camera-space silhouettes do
    // not overlap; the rear row is smaller and sits near the dark backdrop.

    // FRONT ROW -------------------------------------------------------
    // Left / center / right main triangles.
    const vec P0(5.30f, 2.35f, -7.25f);
    const vec P1(6.25f, 2.35f, -7.25f);
    const vec P2(5.775f, 4.05f, -7.25f);

    const vec M0(6.30f, 2.35f, -7.55f);
    const vec M1(7.25f, 2.35f, -7.55f);
    const vec M2(6.775f, 4.20f, -7.55f);

    const vec N0(7.30f, 2.35f, -7.85f);
    const vec N1(8.25f, 2.35f, -7.85f);
    const vec N2(7.775f, 4.20f, -7.85f);

    // The main triangles retain the cool metallic treatment but are kept
    // below the previous nearly-white response.
    extra_materials[m] = new metal(vec(0.68f, 0.72f, 0.79f), 0.085f);
    final_shapes[m] = new triangle(P0, P1, P2, extra_materials[m]); ++m;

    extra_materials[m] = new metal(vec(0.65f, 0.70f, 0.78f), 0.095f);
    final_shapes[m] = new triangle(M0, M1, M2, extra_materials[m]); ++m;

    extra_materials[m] = new metal(vec(0.67f, 0.72f, 0.80f), 0.09f);
    final_shapes[m] = new triangle(N0, N1, N2, extra_materials[m]); ++m;

    // REAR / INSET ROW -----------------------------------------------
    // Smaller darker triangles are placed just in front of the backdrop.
    // Their x positions compensate for the deeper z plane, so they read as
    // inset facets rather than as a second set of floating white triangles.
    const vec P3(5.10f, 2.35f, -8.15f);
    const vec P4(5.67f, 2.35f, -8.15f);
    const vec P5(5.385f, 3.45f, -8.15f);

    const vec M3(6.23f, 2.35f, -8.15f);
    const vec M4(6.80f, 2.35f, -8.15f);
    const vec M5(6.515f, 3.55f, -8.15f);

    const vec N3(7.36f, 2.35f, -8.15f);
    const vec N4(7.93f, 2.35f, -8.15f);
    const vec N5(7.645f, 3.55f, -8.15f);

    extra_materials[m] = new metal(vec(0.36f, 0.40f, 0.48f), 0.14f);
    final_shapes[m] = new triangle(P5, P4, P3, extra_materials[m]); ++m;

    extra_materials[m] = new metal(vec(0.38f, 0.43f, 0.51f), 0.14f);
    final_shapes[m] = new triangle(M5, M4, M3, extra_materials[m]); ++m;

    extra_materials[m] = new metal(vec(0.40f, 0.45f, 0.54f), 0.14f);
...
__global__ void create_volumetrics(
    ...
    fog_media[idx] = new constant_medium(fog_boundaries[idx], 0.008f, fog_materials[idx]);
    ++idx;
